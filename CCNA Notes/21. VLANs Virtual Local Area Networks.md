...
- Then we configure our WAN Routing
- On the switch
  - (config)#interface fe0/1
  - (config-if)#_no switchport_ This converts the physical port to a layer 3 port
  - (config-if)#_ip address 10.10.100.1 255.255.255.0_
  - (config)#_ip route 0.0.0.0 0.0.0.0 10.10.100.2_
- On the router
  - (config)#_Interface fe0/1_
  - (config-if)#_ip address 10.10.100.2 255.255.255.0_
  - (config-if)#_interface fe0/2_
  - (config-if)#_ip address 203.0.113.1 255.255.255.0_
  - (config)#_ip route 0.0.0.0 0.0.0.0 203.0.113.2_
  - (config)#_ip route 10.10.0.0 255.255.0.0 10.10.100.1_ 
