...
This can be done with _Start_ as well

# Upgrading the IOS System Image

IOS softwre images can be downloaded from https://software.cisco.com/

After downloading the software, copy to the device's flash using TFTP
  - Copy TFTP Flash

Delete the old system image or use the _boot system_ command
- _Boot System_ command must be entered at global configuartion. (Config T)
- _Boot System Flash_ (File Name)
- Then save the changes to the startup-config. _Copy Run Start_
