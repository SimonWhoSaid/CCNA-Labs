...
- Because they don't have any physica attributes which can fail, loopback interfaces never go down
...
- The loopback ios commonly used for traffic that terminates on the router itself
- This could be management traffic, VoIP, BGP peering
- This provides redundancy if htere are multiple paths to the router
- The loopback is also used to identify the router in OSPF
- Loopback on a Router can be advertised

To create a loop back address you must be in config t, _interface loopback 0_
Then it will take you into interface mode which you can configure with _ip address [IP ADDRESS] 255.255.255.255

We configure loopback interfaces because it can advertise the IP address with the routing protocol. Because it is a logical address that means anything that were to happen to the router's connection physically, such as a port being down or cable not working, it will still have a way to connect to the loopback address. Loopback addressing helps with redundancy when there are multiple paths available.
...
- When a matching peer is found, the routers form an adjacency with each other, then they exchange routing information
...
Adjancies happen when a routing protocol is enabled on an interface, so the router advertises itself with that routing protocol to its neighbors that are also running the same protocol.
...
- In this example Fe2/0 is configured as a passive interface, RA and RB will learn routes to 10.0.2.0, but internal network information will not be sent to RC
  
<img width="588" height="208" alt="image" src="https://github.com/user-attachments/assets/03cbd6ea-f9a5-4fb0-a4e9-d6411f7a5624" />
...
All in all Passive interfaces are commonly used for loopback interfaces and when we do not want to send routing information out but we do want our internal devices to know about the link.
...
- Routers learn routes by:
  - An administrator configuring IP addresses on its interfaces
  - An admin configuring static routes on that router
  - Receiving them via routing protocols from neighbor routers

## Router Precendence

A Router's main job is to forward packets received towrds their destination, based on the packet's destination IP address.
It decided wehre to forward a packet based on th routes it has learned
...
Only the best route for each elarned network/prefix makes it into the routing table and is used to forward packets. The criteria for which is the ebst route to the exact same network/prefix is first, Administrative Distance (lower value is better), then the Metric (once again lower value).

If there are multiple best routes with the same AD and Metric, they will all enter the routing table and the router will perform Equal Cost Load Balancing over them.

Double check the Routing Protocols, Longest Prefix, AD and Metrics, with _show ip route <destination IP address>_
